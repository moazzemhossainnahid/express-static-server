...
exports.addABlog = async (req, res) => {
    try {
        ...
        console.log(req.file);
        // if (req.file) {
        //     blog.banner = `http://localhost:5000/images/${req.file.filename}`;
        // }
        if (req.file) {
            blog.video = `http://localhost:5000/videos/${req.file.filename}`;
        }
        const createdUser = await Blogs.create(blog);
        res.send({
            success: true,
